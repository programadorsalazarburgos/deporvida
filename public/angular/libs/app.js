angular.module('DemoPagineo', []).controller('SupplierController', ['$scope',function($scope, $http, API_URL) {
...
$scope.getEmployes = function(){
 $.ajax({
     url: base + '/api/supplier',
     type: 'GET',
     dataType: 'JSON',

 }).success(function(response){
    $scope.usuarios3 = response;
    $scope.configPages = function() {
        ...
        if (ini < 1) {
            ini = 1;
            if (Math.ceil($scope.usuarios3.length / $scope.pageSize) > 10)
                fin = 10;
            else
                fin = Math.ceil($scope.usuarios3.length / $scope.pageSize);
        }
        else {
            if (ini >= Math.ceil($scope.usuarios3.length / $scope.pageSize) - 10) {
                ini = Math.ceil($scope.usuarios3.length / $scope.pageSize) - 10;
                fin = Math.ceil($scope.usuarios3.length / $scope.pageSize);
            }
        }
        if (ini < 1) ini = 1;
