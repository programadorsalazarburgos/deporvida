        ...
        success: function (data)
        {
            if (data.validate)
            {
                ...
                $.each(data.data.ciudadano, function (index, value)
                {
                    ...
                    if(index==='id_procedencia_departamento')
                    {
                        departamentos();
                    }
                    if(index==='id_procedencia_municipio')
                    {
                        $('#persona_id_procedencia_municipio').val(value).trigger('change');
                    }

                });
                $.each(data.data.grupo_poblacional, function (index, value)
                {
                    $('#id_grupo_poblacional_'+ value.id).prop('checked', true);
                });
                $.each(data.data.grupo_poblacional, function (index, value)
                {
                    $('#id_grupo_poblacional_'+ value.id).prop('checked', true);
                });
                $.each(data.data.discapacidad, function (index, value)
                {
                    $('#id_discapacidad_'+ value.id_discapacidad).prop('checked', true);
                });
                $.each(data.data.disciplina, function (index, value)
                {
                    $('#id_disciplina_'+ value.id_disciplina).prop('checked', true);
                });
...
{
    var result=true;
    if($('#persona_fecha_nacimiento').val()=='')
    {
        result=false;
        $("#persona_fecha_nacimiento").focus(); 
    }
    if(typeof($('#persona_residencia_direccion').attr('readonly'))!=='undefined')
    {
        if($('#persona_residencia_direccion').val()=='')
        {
            result=false;
            $("#persona_residencia_direccion").focus();
        }
    }
    $("#empleado_form").valid()?result=result:result=false;
    return result;
}
function guardar()
{
...
$(function ()
{
    ...
    $('#ficha_id_vereda').change(function ()
    {
        if ($(this).val() !== '')
        {
            ...
            $('#div_direccion').hide();
            $('#persona_residencia_direccion').removeAttr('readonly');
        ...
        } else
        {
            ...
            $('#persona_residencia_direccion').attr('readonly', 'readonly');
    ...
    $('.regimen_'+$('#ficha_id_tipo_afiliacion').val()).removeAttr('style');
});
function EPS_regimen() 
{
    $('#ficha_id_tipo_afiliacion').change(function()
    {
        $('#ficha_id_eps').val('');
        $('.regimenes').attr('style','display:none');
        $('.regimen_'+$(this).val()).removeAttr('style');
    });
}